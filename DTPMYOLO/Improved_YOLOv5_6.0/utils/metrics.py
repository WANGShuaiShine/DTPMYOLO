...
# 其实在yolov5源码中设置是有GIoU, DIoU, CIoU这些边界框iou损失，但是默认值都为False。
# 这里我们需要将原始的bbox_iou函数代码注释掉，替换成我们自己的算法；
# 为了方便实验测试和后期扩展功能，这里我将EIoU、Alpha-IoU、SIoU、Focal-EIOU这几个功能集中在一起；
# 如果想要使用不同的Iou计算边界框损失，只需要修改utils/loss.py下的iou方法即可。
...
# 这里是我的代码实现部分
# gamma参数是Focal EloU中的gamma参数，一般就是为0.5，可以自行更改。
# alpha参数为Alpha-IOU中的alpha参数，默认为1，即使用原始I0U。若需要使用Alpha-IOU，只需将其设置为任意值
# 为了方便实验测试和后期扩展功能，这里我将EIoU、Alpha-IoU、SIoU、Focal-EIOU这几个功能集中在一起；
# 如果想要使用不同的Iou计算边界框损失，只需要修改utils/loss.py下的iou方法即可。
