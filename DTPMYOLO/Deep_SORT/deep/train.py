...
# 只需要修改该train.py下第14行代码，存放数据集路径，要放绝对路径。（其他参数默认就行）ROOT /deep_sort_pytorch/deep_sort/deep/Market-1501
parser = argparse.ArgumentParser(description="Train on market1501")
parser.add_argument("--data-dir", default='G:/BaiduNetdiskDownload/Yolov5_DeepSort_Pytorch/deep_sort_pytorch/deep_sort/deep/Market-1501', type=str)
...
def main():
    for epoch in range(start_epoch, start_epoch+300):       # 第197行，修改数字‘300’，改成自己训练的轮数。
...
# 直接运行train.py就可以训练了。训练结束后会在deep_sort_pytorch/ deep_sort/ deep/checkpoint文件夹下生成一个跟踪权重ckpt.t7。（代码中附带了一个预训练跟踪权重）
